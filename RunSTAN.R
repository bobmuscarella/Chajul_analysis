setwd("") # the directory where Bob.stan is


dataSTAN<-list(growth = growth ,species = species,nspecies=max(as.numeric(as.factor(as.character(species)))), plot = plot,nplot = max(as.numeric(as.factor(as.character(plot)))), indiv = indiv,nindiv = max(as.numeric(as.factor(as.character(indiv)))), n = n ,trait=trait,dbh=dbh,plotagb=plotagb)

library(rstan)
fit<-stan(file="Bob.stan",data=dataSTAN,iter=3000,chains=2)
